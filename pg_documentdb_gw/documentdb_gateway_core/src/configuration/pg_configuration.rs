...
pub struct PgConfiguration {
    ...
    /// Last successfully-read `max_connections` GUC value, used as a fallback
    /// when a subsequent read fails so we don't fabricate a divergent value
    /// (which would change the pool-cache key and orphan healthy pools).
    last_good_max_connections: AtomicUsize,
}
...
impl PgConfiguration {
    ...
    pub async fn new(
        ...
        let mut configuration = Arc::new(Self {
            ...
            refresh_task: None,
            watch_task: None,
            last_good_max_connections: AtomicUsize::new(0),
        });
    ...
    fn max_connections(&self) -> usize {
        ...
        match max_connections {
            n if n < 0 => {
                ...
                // Reuse the last successfully-read value instead of fabricating a
                // divergent one. `max_connections` is part of the pool-cache key, so
                // returning a different number here would orphan otherwise-healthy
                // pools. Fall back to 25 only on the first-read edge case.
                let last_good = self.last_good_max_connections.load(Ordering::Relaxed);
                if last_good > 0 {
                    last_good
                } else {
                    25usize
            ...
            n => {
                // `n >= 0` always converts to usize, so the fallback is unreachable.
                let value = usize::try_from(n).unwrap_or(25);
                self.last_good_max_connections
...
mod tests {
    ...
    /// Constructs a `PgConfiguration` directly with a settable `values` map,
    /// bypassing `PgConfiguration::new` (which would query Postgres). No refresh
    /// or watch tasks are spawned, so `max_connections()` reads exactly what we set.
    ...
    #[tokio::test]
    async fn max_connections_keeps_last_known_good_on_bad_read() {
        // Async context is required only to build the (lazy) connection pools.
        yield_now().await;

        let mut values = HashMap::new();
        values.insert("max_connections".to_owned(), "100".to_owned());
        let config = test_configuration(values);

        // Valid read: real value is returned and remembered as last-known-good.
        assert_eq!(config.max_connections(), 100);

        // GUC now missing (bad read): must return the last-known-good value (100),
        // NOT the fabricated 25 that used to orphan healthy pools.
        config.values.store(Arc::new(HashMap::new()));
        assert_eq!(config.max_connections(), 100);
    }

    #[tokio::test]
    async fn max_connections_falls_back_to_25_without_prior_good_value() {
        yield_now().await;

        // Bad read with no previously-recorded good value: first-read edge case.
