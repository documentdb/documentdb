...
use documentdb_gateway_core::configuration::{
    DynamicConfiguration, PgConfiguration, Version, DEFAULT_MAX_WIRE_VERSION,
...
async fn defaults_match_guc_boot_values() -> TestResult {
    ...
    assert_eq!(config.server_version(), Version::Seven);
    assert_eq!(config.server_version().as_str(), DEFAULT_SERVER_VERSION);
