...
```bash
   ...
   # docker run -dt -p 10260:10260 --name documentdb-container documentdb --username <YOUR_USERNAME> --password <YOUR_PASSWORD> --enable-tls-enforcement
...
By default, `documentdb-local` accepts both plain and TLS connections.

If you started `documentdb-local` with `--enable-tls-enforcement`, use:
