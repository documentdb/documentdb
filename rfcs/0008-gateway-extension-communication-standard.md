|rfc	|title	|status	|owner	|issue	|discussion	|version-target	|implementations	|
|---	|---	|---	|---	|---	|---	|---	|---	|
|0	|Standardization of Gateway / Extension Interaction	|
Draft	|@seoc01	|
https://github.com/documentdb/documentdb/issues/XXX	|
https://github.com/documentdb/documentdb/discussions/XXX	|1	|	|
|---	|---	|---	|---	|---	|---	|---	|---	|

https://github.com/documentdb/documentdb/pull/XXX


# RFC-XXXX: Standardization of Gateway / Extension Interaction
...
The OP_MSG must send exactly one type 0 section that contains the main body of the command, it is sent as a single BSON object, it must contain all arguments for the command and the global command arguments, except for arguments that are explicitly allowed in the Type 1 sequence based on the command. These arguments are:
...
With the current approach each command would need a custom implementation to be able to re-construct the original command, and some data for some commands, such as lsid, is currently unavailable. 
...
This is not preferred because it would require additional work at the Gateway layer, where the Gateway may need to create a new several Mb message before passing to the extension, and would prevent the extension from being able to validate the command message is under 16Mb without, .
...
Must call a **documentdb_api** function with only a single required argument
