...
Optional arguments:
  ...
  --enable-tls-enforcement
                        ...
                        When set, plain (non-TLS) connections are rejected.
                        Overrides ENABLE_TLS_ENFORCEMENT environment variable.
EOF
}
...
do
  case $1 in
    ...
    --enable-tls-enforcement)
        export ENABLE_TLS_ENFORCEMENT=true
        shift;;
...
export ENABLE_TLS_ENFORCEMENT=${ENABLE_TLS_ENFORCEMENT:-false}
...
if [ -n "$ENABLE_TLS_ENFORCEMENT" ] && \
   [ "$ENABLE_TLS_ENFORCEMENT" != "true" ] && \
   [ "$ENABLE_TLS_ENFORCEMENT" != "false" ]; then
    echo "Invalid enable-tls-enforcement value $ENABLE_TLS_ENFORCEMENT, must be true or false"
    exit 1
fi
...
if [ "$ENABLE_TLS_ENFORCEMENT" = "true" ]; then
    echo "Enabling TLS enforcement..."
    jq '.EnforceTls = true' "$configFile" > "$configFile.tmp" && \
    mv "$configFile.tmp" "$configFile"
else
    echo "TLS enforcement disabled by default — accepting both plain and TLS connections..."
