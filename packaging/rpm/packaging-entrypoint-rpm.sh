...
# DOCUMENTDB_VERSION is required and must be provided by caller
if [[ -z "${DOCUMENTDB_VERSION:-}" ]]; then
    echo "ERROR: DOCUMENTDB_VERSION environment variable not set" >&2
    exit 1
fi
echo "Using DOCUMENTDB_VERSION: ${DOCUMENTDB_VERSION}"
