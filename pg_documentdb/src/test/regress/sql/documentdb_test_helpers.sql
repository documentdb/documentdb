...
-- and ensure it has entered its main loop (past dependency waiting).
...
BEGIN
  -- First, wait for the background worker to appear in pg_stat_activity
  LOOP
  ...
  -- Next, verify the background worker role exists. The background worker waits for this
  -- role before entering its main loop. Once the role exists, we know the worker is ready.
  LOOP
    SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = 'documentdb_bg_worker_role') INTO v_role_exists;
...
-- Wait for background worker to be fully ready and signal it to wake immediately.
-- This avoids needing a fixed 10-second sleep matching LatchTimeOutSec.
CREATE OR REPLACE PROCEDURE documentdb_test_helpers.wait_for_background_worker_ready()
AS $$
BEGIN
  -- First ensure the background worker is running and has entered its main loop
  CALL documentdb_test_helpers.wait_for_background_worker();

  -- Send SIGHUP to all backends (including background worker) to wake it immediately.
  -- The background worker's SIGHUP handler sets its latch, breaking out of WaitLatch.
  PERFORM pg_reload_conf();

  -- Brief sleep to allow one worker iteration to complete
  PERFORM pg_sleep_for('2000 ms');
END
$$
LANGUAGE plpgsql;
...
-- Change the active status of index build jobs registered on pg_cron
