...
--
-- The dynamic cursor therefore forces a de-duplicating bitmap scan whenever the
-- composite index is multikey. This safeguard is gated by the
-- enable_dynamic_cursor_multikey_bitmap GUC (default on); this test exercises both
-- states.
...
------------------------------------------------------------------------------
-- Default (GUC on): the multikey index forces a de-duplicating bitmap scan
...
------------------------------------------------------------------------------
-- GUC off: the safeguard is disabled, so the cursor takes the ordered Secondary
-- Index Scan (dc.type = 3). Draining the same query re-emits the multikey
-- document once per array element, so the total exceeds 201 (203 here). This
-- documents the behavior the default-on GUC prevents.
------------------------------------------------------------------------------
